...
class _PetCreateFlowPageState extends ConsumerState<PetCreateFlowPage> {
  ...
  Widget build(BuildContext context) {
    ...
    _customSpeciesCtrl.value = _customSpeciesCtrl.value.copyWith(text: state.customSpeciesName);
    _customBreedCtrl.value = _customBreedCtrl.value.copyWith(text: state.customBreedName);
    _customPatternCtrl.value = _customPatternCtrl.value.copyWith(text: state.customPatternName);
    _microchipCtrl.value = _microchipCtrl.value.copyWith(text: state.microchipId);
      ...
      body: catalogAsync.when(
        ...
        data: (catalog) {
          ...
          final breedsForSpecies = state.speciesId == null
              ? catalog.breeds
              : catalog.breeds.where((e) => e.speciesId == state.speciesId).toList(growable: false);
                ...
                children: <Widget>[
                  ...
                  if (state.step == PetCreateStep.basic) ...[
                    ...
                    if (state.speciesMode == CatalogPickMode.catalog)
                      DropdownButtonFormField<String>(
                        ...
                        decoration: const InputDecoration(labelText: 'Выберите вид'),
                        items: species
                  ...
                  if (state.step == PetCreateStep.breed) ...[
                    ...
                    if (state.breedMode == CatalogPickMode.catalog)
                      DropdownButtonFormField<String>(
                        value: state.breedId,
                        decoration: const InputDecoration(labelText: 'Выберите породу'),
                  ...
                  if (state.step == PetCreateStep.appearance) ...[
                    ...
                    if (state.patternMode == CatalogPickMode.catalog)
                      DropdownButtonFormField<String>(
                        ...
                        decoration: const InputDecoration(labelText: 'Выберите паттерн'),
                        items: patterns
                            .map((e) => DropdownMenuItem<String>(value: e.id, child: Text(e.name)))
                            .toList(growable: false),
                    ...
                    Wrap(
                      ...
                      children: <Widget>[
                        ...
                        ActionChip(
                          ...
                          onPressed: () => _openColorPicker((hex) => c.addCustomColor(hex)),
                        ),
                      ],
                    ),
                    const SizedBox(height: PawlySpacing.lg),
                  ...
                  if (state.step == PetCreateStep.optional) ...[
                    Text('Необязательные поля', style: Theme.of(context).textTheme.titleLarge),
                    const SizedBox(height: PawlySpacing.sm),
                    PawlyButton(
                      ...
                      onPressed: () => _pickDate(c.setBirthDate, initial: state.birthDate),
                    ),
                    const SizedBox(height: PawlySpacing.sm),
                    DropdownButtonFormField<String>(
                      value: state.isNeutered,
                      decoration: const InputDecoration(labelText: 'Стерилизация'),
                      items: const <DropdownMenuItem<String>>[
                    ...
                    Row(
                      children: <Widget>[
                        ...
                        Expanded(
                          child: PawlyButton(label: 'Далее', onPressed: c.nextStep),
                        ),
                      ],
                    ),
                  ],
                    ...
                    PawlyCard(
                      child: Column(
                        ...
                        children: <Widget>[
                          Text('Имя: ${state.name}'),
                          Text('Пол: ${state.sex}'),
                          Text('Вид: ${state.speciesMode == CatalogPickMode.catalog ? "Из каталога" : "Свой"}'),
                          Text('Порода: ${state.breedMode == CatalogPickMode.catalog ? "Из каталога" : "Своя"}'),
                          Text('Паттерн: ${state.patternMode == CatalogPickMode.catalog ? "Из каталога" : "Свой"}'),
                          Text('Цветов: ${state.colorIds.length + state.customColorsHex.length}'),
                        ],
                      ),
                    ),
                    const SizedBox(height: PawlySpacing.lg),
                    PawlyButton(
                      label: state.isSubmitting ? 'Создаем...' : 'Создать питомца',
                      onPressed: state.isSubmitting
                          ? null
                          : () async {
                              final ok = await c.submit(catalog);
...
class _StepPills extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const labels = <String>['База', 'Порода', 'Внешность', 'Опционально', 'Проверка'];
